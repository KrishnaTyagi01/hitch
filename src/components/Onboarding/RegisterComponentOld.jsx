...
const RegisterComponentOld = () => {
	...
	const redirectToProfile = () => {
		if (redirectUser) return <Redirect to='/profile' />;
	};
		...
		registerUser({ email: email, name: name, password: password })
			.then((res) => {
				if (res.status === 'success') {
					authenticate(
						{ name: res.user.name, username: res.user.username, token: res.token },
						() => {
...
export default RegisterComponentOld;
